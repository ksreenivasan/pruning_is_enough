...
class TinyResNetNormal(nn.Module):

    def __init__(self, block, layers, num_classes=200, base_width=64):
        super(TinyResNetNormal, self).__init__()
...
def _resnet(block, layers, pretrained):
    return TinyResNetNormal(block, layers)


def TinyResNet18Normal(pretrained=False):
    return _resnet(BasicBlock, [2, 2, 2, 2], pretrained)
