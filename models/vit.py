...
# we modify these files locally to apply pruning to ViT
from .vit_utils.patch_embed import PatchEmbed
from .vit_utils.mlp import Mlp
from .vit_utils.drop import DropPath
...
class Attention(nn.Module):
    def __init__(self, builder, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.qkv = builder.linear(dim, dim * 3)        # NOTE: parser_args.bias is being used implicitly here (so qkv_bias will be ignored)
...
class LayerScale(nn.Module):
    def __init__(self, dim, init_values=1e-5, inplace=False):
        ...
        self.gamma = nn.Parameter(init_values * torch.ones(dim))        # TODO: freeze this value
...
class Block(nn.Module):
    ...
    def __init__(
            self, builder, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()

        self.norm1 = norm_layer(dim)
        self.attn = Attention(builder, dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        ...
        self.ls2 = LayerScale(builder, dim, init_values=init_values) if init_values else nn.Identity()
...
class ParallelBlock(nn.Module):     # TODO: builder must be added for parallelization!
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = embed_layer(builder, img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))     # NOTE be sure to freeze!
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))     # NOTE be sure to freeze!
        self.pos_drop = nn.Dropout(p=drop_rate)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.blocks = nn.Sequential(*[
            block_fn(
                builder, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
        ...
        self.head = builder.linear(final_chs, num_classes) if num_classes > 0 else nn.Identity()
...
def init_weights_vit_timm(module: nn.Module, name: str = ''):
    ...
    if isinstance(module, nn.Linear):           # NOTE: SubnetLinear will be re-initialized here!
